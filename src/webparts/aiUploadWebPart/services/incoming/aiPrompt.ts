...
export function buildIncomingUserPrompt(
  ...
  } else if (signature && signature.senderName) {
  ...
  if (refNo && refNo.trim()) {
    parts.push('', letterType === 'email'
      ? 'Detected Ref No from Our Ref: in the last email or the following letter:'
      : 'Detected Ref No from Our Ref: or standalone Ref:', refNo.trim());
  } else if (letterType === 'email') {
